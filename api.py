...
@app.get("/introspection", tags=["brain"])
async def get_introspection():
    """
    Full self-awareness dashboard: goal metrics, category success rates,
    quality scores, latency, latest intelligence briefing, and capability gaps.
    """
    if not _bot:
        raise HTTPException(status_code=503, detail="Bot is not running")
    return await _bot.introspection.get_dashboard()


@app.get("/introspection/briefing", tags=["brain"])
async def get_briefing():
    """Return the latest intelligence briefing generated by IntrospectionBrain."""
    if not _bot:
        raise HTTPException(status_code=503, detail="Bot is not running")
    briefing = _bot.introspection.get_briefing()
    if not briefing:
        return {"status": "no briefing yet — will generate after 5 minutes of operation"}
    return briefing


@app.get("/introspection/gaps", tags=["brain"])
async def get_capability_gaps():
    """Capability gap proposals — things Gigia can't do that it needs."""
    if not _bot:
        raise HTTPException(status_code=503, detail="Bot is not running")
    async with aiosqlite.connect(_DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        try:
            rows = await (await db.execute(
                "SELECT * FROM capability_gaps ORDER BY created_at DESC LIMIT 20"
            )).fetchall()
            gaps = []
            for r in rows:
                row = dict(r)
                try:
                    row["proposal"] = json.loads(row["proposal"])
                except Exception:
                    pass
                gaps.append(row)
